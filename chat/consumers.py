...
class ChatConsumer(ObserverModelInstanceMixin, GenericAsyncAPIConsumer):
    ...
    async def create_message(self, message, **kwargs):
        room: Conversation = await self.get_room(pk=self.room_subscribe)
    ...
    async def notify_users(self):
        room: Conversation = await self.get_room(self.room_subscribe)
    ...
    def get_room(self, pk: int) -> Conversation:
    ...
    def add_user_to_room(self, pk):
        ...
        if not user.current_rooms.filter(pk=self.room_subscribe).exists():
            user.current_rooms.add(Conversation.objects.get(pk=pk))
